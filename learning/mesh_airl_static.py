    ...
    def cat_all_feature(self, state, context, idx, channel):
        ...
        if self.use_encoder:
            for bs in range(state.shape[0]):
                ...
                compressed = [compressed[tmp_channel][0] if type(compressed[tmp_channel]) is tuple else compressed[tmp_channel] for tmp_channel in range(self.output_channel)]  # [tensor((2d+1)^2, emb_dim)]
